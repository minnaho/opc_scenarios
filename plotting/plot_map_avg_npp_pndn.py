...
filename = 'avg_fullts_int_avg_100m_50m_'
...
# control scenario
cntrl_nc = outpath+filename+'PNDN_only_'+varstr+'.nc'
...
for t_i in range(len(ncfiles)):
    datanc = np.squeeze(Dataset(ncfiles[t_i],'r')[var_nc])
    ...
    if 'PNDN' in ncfiles[t_i]:
        p_plot1 = ax.flat[t_i].pcolormesh(lon_nc,lat_nc,varplt,transform=ccrs.PlateCarree(),cmap=c_map1,vmin=0,vmax=0.002)
    else:
...
savename = 'map_100m_int_'+varstr+'_'+region_name+'_'+exp[-1]+'_pndn_abs_fullts.png'
