...
if var_nc == 'rho':
    clines = np.arange(1022.5,1040.5,0.5)
...
c25 = np.arange(1026.25,1028,0.25)
clines = list(np.arange(1022.5,1026.5,0.5))+list(c25)
...
if loc == 'HTP':
    ...
    #ind_en = -118.47
    ind_st = -119.5 # range of lon to plot over
    ind_en = -118.47
    
    #dp_st = -200 # set depth of domain
    dp_st = -600 # set depth of domain
    dp_en = 0
...
for n_i in range(len(ncfile)):
    ...
    if var_nc == 'rho':
        ...
        rho_cntrl = np.array(Dataset(cntrlnc[n_i],'r').variables['rho'][0,:,y_site,:])+rho0
        rho_fulll = np.array(Dataset(fulllnc[n_i],'r').variables['rho'][0,:,y_site,:])+rho0
    ...
    clinecolor = 'k'
    c_plt_cntrl = ax.flat[0].contour(lon_reshape[:,ind_st_p:ind_en_p],z_r_cntrl[:,ind_st_p:ind_en_p],rho_cntrl[:,ind_st_p:ind_en_p],clines,colors=clinecolor,linewidths=1,fmt=fmtc)
    c_plt_fulll = ax.flat[1].contour(lon_reshape[:,ind_st_p:ind_en_p],z_r_fulll[:,ind_st_p:ind_en_p],rho_fulll[:,ind_st_p:ind_en_p],clines,colors=clinecolor,linewidths=1,fmt=fmtc)
