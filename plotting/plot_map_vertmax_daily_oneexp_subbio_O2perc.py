...
filename2d = 'int_100m_'+exp2[0]+'_biomass_'
filename2c = 'int_100m_cntrl_biomass_'
...
for y_i in range(start_year,end_year+1):
    ...
    for m_i in range(s_m,e_m):
        ...
        for d_i in list(range(1,ndays+1)):
            ...
            vardnc1 = np.squeeze(datanc.variables[var_nc1])

            vardnc1[vardnc1>1E10] = np.nan
            vardnc1[vardnc1<=0] = np.nan
            # get vertical maximum
            vardplt1 = np.nanmax(vardnc1,axis=0)

            # var2 - biomass 
            # subtract cntrl scenario
            varrnc2 = np.squeeze(Dataset(outpath2+filename2d+dtstr+'.nc','r').variables[var_nc2])
            varrnc2[varrnc2>1E10] = np.nan
            varrnc2[varrnc2<=0] = np.nan

            varcnc2 = np.squeeze(Dataset(outpath2+filename2c+dtstr+'.nc','r').variables[var_nc2])
            ...
            #vardplt3 = np.nanmin(vardnc3,axis=0)
            # vertical lowest 10th percentile
            vardplt3 = np.nanpercentile(vardnc3,10,axis=0)
            ...
            savename = 'map_vertmax_subbio_O2perc10_'+region_name+'_'+exp[-1]+'_'+dtstr+'.png'
