...
for y in range(start_year,end_year+1):
    ...
    for m in range(s_m,e_m):
        ...
        cb_ax1 = fig.add_axes([p0[2]-.005,p1[1]-.001,.01,p0[3]-p1[1]-.001])
        ...
        for e_i in range(len(exp)):
            ...
            if perc == True:    
                varplt = ((vardplt - varcplt)/varcplt)*100
            else:    
                varplt = vardplt - varcplt

            # plot maps
            p_plot = ax.flat[e_i+1].pcolormesh(lon_nc,lat_nc,varplt,transform=ccrs.PlateCarree(),cmap=c_map,vmin=v_min,vmax=v_max)
