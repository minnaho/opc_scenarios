...
if var_nc == 'var':
    if perc == True:
        ...
        v_max = 5*mmolm3_to_mgl
        v_min = -5*mmolm3_to_mgl
...
for t_i in range(len(ncfiles)):
    ...
    if 'PNDN' in ncfiles[t_i]:
        p_plot1 = ax.flat[t_i].pcolormesh(lon_nc,lat_nc,varplt,transform=ccrs.PlateCarree(),cmap=c_map1,vmin=5,vmax=9)
    else:
...
cb_ax = fig.add_axes([p0[2]+.015,p1[1],.01,p1[3]-p1[1]])

cb = fig.colorbar(p_plot,cax=cb_ax,orientation='vertical')
cb.set_label(cblabel,fontsize=axis_tick_size)
cb.ax.tick_params(axis='both',which='major',labelsize=axis_tick_size)

savename = 'map_avg_'+str(dp_avg0*2)+'_'+str(dp_avg1*2)+'_'+varstr+'_'+region_name+'_'+exp[-1]+'_minus_PNDN_abs.png'
