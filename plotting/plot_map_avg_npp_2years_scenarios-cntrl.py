...
# subtract from cntrl
cntrld = np.squeeze(Dataset(cntrl_nc,'r')[var_nc])*86400
...
for e_i in range(2,len(exp)):
    ...
    #varplt = dataavg
    #p_plot = ax.flat[e_i-2].pcolormesh(lon_nc,lat_nc,varplt,transform=ccrs.PlateCarree(),cmap=c_map)
    ...
    p_plot = ax.flat[e_i-2].pcolormesh(lon_nc,lat_nc,varplt,transform=ccrs.PlateCarree(),cmap=c_map,norm=mcolors.DivergingNorm(vmin=v_min,vcenter=0,vmax=v_max))
...
savename = 'map_2years-cntrl_100m_int_'+varstr+'_'+region_name+'_abs_'+timep+'.png'
