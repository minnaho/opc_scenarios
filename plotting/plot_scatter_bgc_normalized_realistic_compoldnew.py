...
for n_i in range(len(exp)):
    ...
    if 'real' in exp[n_i]:
        ax.bar(n_i,roms_neg-cntrl_mean_new,bottom=cntrl_mean_new,color='gray')
    else:
        ax.bar(n_i,roms_neg-cntrl_mean_old,bottom=cntrl_mean_old,color='white',edgecolor='k')

ax.set_ylim(bottom=cntrl_mean_new-(cntrl_mean_new*0.01),top=fulll_mean_old+(cntrl_mean_new*0.01))
