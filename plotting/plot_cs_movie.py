...
freshpath = '/data/project6/ROMS/L2SCB_AP/fresh/daily/'
nutripath = '/data/project6/ROMS/L2SCB_AP/nutrients/daily/'
cntrlpath = '/data/project6/ROMS/L2SCB_1997_2000/DAILY/'
#fulllpath = '/data/project6/ROMS/L2SCB_AP/daily/'
fulllpath = '/data/project6/ROMS/L2SCB_P_1999_2000/daily/'
...
for y in range(start_year,end_year+1):
    ...
    for m in range(s_m,e_m):
        ...
        for d in list(range(1,ndays+1)):
            ...
            savename.append('cs_'+loc+'_'+var_nc+'_'+year_month+'.png')
...
for n_i in range(len(ncfile)):
    freshnc.append(freshpath+ncfile[n_i])
    nutrinc.append(nutripath+ncfile[n_i])
...
if loc == 'HTP':
    ...
    lon_site = -118.529712
    ind_st = -118.6 # range of lon to plot over
    ind_en = -118.46
    
    dp_st = -80 # set depth of domain
    dp_en = 0
...
for n_i in range(len(ncfile)):
    ...
    if var_nc == 'rho':
        ...
        roms_var_fresh = np.array(Dataset(freshnc[n_i],'r').variables[var_nc][0,:,y_site,:])+rho0
        roms_var_nutri = np.array(Dataset(nutrinc[n_i],'r').variables[var_nc][0,:,y_site,:])+rho0
    ...
    if var_nc == 'biomass':
        ...
        roms_var_fresh_dtc = np.array(Dataset(freshnc[n_i],'r').variables['DIATC'][0,:,y_site,:])
        roms_var_fresh_dtc[roms_var_fresh_dtc>1E10] = 0
        roms_var_fresh_spc = np.array(Dataset(freshnc[n_i],'r').variables['SPC'][0,:,y_site,:])
        roms_var_fresh_spc[roms_var_fresh_spc>1E10] = 0
        roms_var_fresh_dzc = np.array(Dataset(freshnc[n_i],'r').variables['DIAZC'][0,:,y_site,:])
        roms_var_fresh_dzc[roms_var_fresh_dzc>1E10] = 0

        roms_var_nutri_dtc = np.array(Dataset(nutrinc[n_i],'r').variables['DIATC'][0,:,y_site,:])
        roms_var_nutri_dtc[roms_var_nutri_dtc>1E10] = 0
        roms_var_nutri_spc = np.array(Dataset(nutrinc[n_i],'r').variables['SPC'][0,:,y_site,:])
        roms_var_nutri_spc[roms_var_nutri_spc>1E10] = 0
        roms_var_nutri_dzc = np.array(Dataset(nutrinc[n_i],'r').variables['DIAZC'][0,:,y_site,:])
        ...
        roms_var_fresh = np.array(Dataset(freshnc[n_i],'r').variables[var_nc][0,:,y_site,:])
        roms_var_nutri = np.array(Dataset(nutrinc[n_i],'r').variables[var_nc][0,:,y_site,:])
    ...
    z_r_fresh = depths.get_zr_zw_tind(Dataset(freshnc[n_i],'r'),grid_nc,0,[y_site-1,y_site+1,0,Dataset(freshnc[n_i],'r').variables['temp'].shape[3]])[0][:,1,:]
    z_r_nutri = depths.get_zr_zw_tind(Dataset(nutrinc[n_i],'r'),grid_nc,0,[y_site-1,y_site+1,0,Dataset(nutrinc[n_i],'r').variables['temp'].shape[3]])[0][:,1,:]
    ...
    if loc == 'OCSan' or loc == 'JWPCP':
        ...
        ax.flat[2].scatter(lon_site+.006,dpipe,marker='^',facecolors='orange',s=psize)
        ax.flat[3].scatter(lon_site+.006,dpipe,marker='^',facecolors='orange',s=psize)
