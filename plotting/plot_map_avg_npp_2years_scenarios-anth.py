...
for e_i in range(2,len(exp)):
    ...
    # in km2
    npp_pos = np.where(varplt>=5)
    npp_neg = np.where(varplt<=-5)
    ...
    print('area >5 mmol m-2 d-1 NPP: '+str(int(pos_area))+' km^2$')
    print('area <-5 mmol m-2 d-1 NPP: '+str(int(neg_area))+' km^2$')
    ...
    c_plot = ax.flat[e_i-2].contour(lon_nc,lat_nc,varplt,[5],transform=ccrs.PlateCarree(),colors='red')
    c_plot = ax.flat[e_i-2].contour(lon_nc,lat_nc,varplt,[-5],transform=ccrs.PlateCarree(),colors='blue')
