...
# control scenario
cntrl_nc = outpath+filename+varstr+'_l1617.nc'
...
for t_i in range(len(ncfiles)):
    ...
    if avg == True:
        ...
        varrd = np.squeeze(np.array(datanc['var']))[dp_layer,:,:]
    ...
    if avg == True:
        ...
        cntrlv = np.squeeze(np.array(cntrld['var']))[dp_layer,:,:]
...
if perc == True:
    savename = 'map_'+str(depth)+'m_'+varstr+'_'+region_name+'_'+exp[-1]+'_l1617_perc_fullts.png'
if avg == True:
    savename = 'map_avg_0_200_'+varstr+'_'+region_name+'_'+exp[-1]+'_l1617_perc_fullts.png'
else:
    savename = 'map_'+str(depth)+'m_'+varstr+'_'+region_name+'_'+exp[-1]+'_l1617_abs_fullts.png'
