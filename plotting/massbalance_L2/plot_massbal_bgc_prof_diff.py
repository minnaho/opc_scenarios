...
savename = 'bgc_prof_diff_budget_mask'+'%02d'%maskn+'_'+varstr
...
for d_i in range(dp0,dp1,dpstp):
    ...
    for e_i in range(len(exp)):
        ...
        # cntrl
        if compexp=='cntrl':
            invdif = bgcnc[res] - bgccomp
        # loads1617
        else:
            invdif = bgcnc - bgccomp[res]
        # write to array
...
for e_i in range(len(exp)):
    ...
    ax.set_title('Total BGC '+varstr+' Mask '+str(maskn),fontsize=axisfont)
    ax.invert_yaxis()
