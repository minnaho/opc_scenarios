...
%% bgc
D    =  netcdf.defVar(ncid,'J_O2', 'float', [dimtime]);
netcdf.putAtt(ncid,D,'units','mmol');
netcdf.putAtt(ncid,D,'long_name','Oxygen balance by biology between two time steps (dt, usually = 1 month)');

D    =  netcdf.defVar(ncid,'air_sea_flux', 'float', [dimtime]);
netcdf.putAtt(ncid,D,'units','mmol');
netcdf.putAtt(ncid,D,'long_name','total O2 excgange with the atmospher between two time steps (dt, usually = 1 month)');

D    =  netcdf.defVar(ncid,'bgc', 'float', [dimtime]);
netcdf.putAtt(ncid,D,'units','mmol');
netcdf.putAtt(ncid,D,'long_name','total biological net use/prod of oxygen between two time steps (dt, usually = 1 month)');
