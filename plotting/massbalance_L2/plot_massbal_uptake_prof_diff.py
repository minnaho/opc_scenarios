...
savename = 'sms_prof_diff_budget_mask'+'%02d'%maskn+'_'+varstr
...
for d_i in range(dp0,dp1,dpstp):
    ...
    #smscomp = (np.array(datacomp.variables['J_O2'])*s2d)/(dttcomp*arecomp)
    smscomp = ((np.array(datacomp.variables['uptake_diat'])*s2d)/(dttcomp*arecomp))+((np.array(datacomp.variables['uptake_sp'])*s2d)/(dttcomp*arecomp))
    ...
    for e_i in range(len(exp)):
        ...
        smsnc = ((np.array(datanc.variables['uptake_diat'])*s2d)/(dttnc*arenc))+((np.array(datanc.variables['uptake_sp'])*s2d)/(dttnc*arenc))
        invnc = np.array(datanc.variables['invm'])/volnc
        # take difference (of only indices that match time period)
        # cntrl
        if compexp=='cntrl':
            invdif = smsnc[res] - smscomp
        # loads1617
        else:
            invdif = smsnc - smscomp[res]
        # write to array
...
for e_i in range(len(exp)):
    ...
    ax.set_title('Uptake minus CTRL '+varstr+' Mask '+str(maskn),fontsize=axisfont)
    ax.invert_yaxis()
