...
for e_i in range(len(exp)):
    if 'all' in exp[e_i]:
        ...
        nc1 = np.squeeze(Dataset(outpath+filest1+exp[e_i][:exp[e_i].index('all')-1]+fileen,'r').variables[var_nc])*mask_mult
        nc1[nc1==0] = np.nan
        diff1 = np.nanmean(nc1) - cntrl_mean_old

        nc2 = np.squeeze(Dataset(outpath+filest2+exp[e_i][:exp[e_i].index('all')-1]+'_realistic'+fileen,'r').variables[var_nc])*mask_mult
        nc2[nc2==0] = np.nan
        diff2 = np.nanmean(nc1) - cntrl_mean_new
    
        diffall = np.nanmean((diff1,diff2))
