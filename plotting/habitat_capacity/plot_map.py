################################################
# plot maps of opc scenarios - 6 maps total
# 1 month or season
# 6 OPC scenarios
...
for e_i in range(len(exp)):
    ncfiles.append(outpath+filename+dtstr+'_'+exp[e_i]+compstr+'.nc')
...
for a_i in range(len(ax.flat)):
    ...
    for l_i in range(len(lon_potw)):
        ax.flat[a_i].scatter(lon_potw[l_i],lat_potw[l_i],marker='o',facecolors='none',edgecolors='gray',s=100)
    # other grid stuff
...
#savename = filename+dtstr+exp[-1]+'_l1617.png'
savename = filename+dtstr+exp[-1]+compstr+'.png'
