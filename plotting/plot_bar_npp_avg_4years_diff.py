...
avgp[0] = np.nanmean((avgp1[0],avgp2[0]))
avgp[1] = np.nanmean((avgp1[1],avgp2[1]))
avgp[2] = np.nanmean((avgp1[2],avgp2[2]))
avgp[3] = np.nanmean((avgp1[3],avgp2[3]))
avgp[4] = np.nanmean((avgp1[4]))
avgp[5] = np.nanmean((avgp1[5]))

stdp[0] = np.nanstd((avgp1[0],avgp2[0]))
stdp[1] = np.nanstd((avgp1[1],avgp2[1]))
stdp[2] = np.nanstd((avgp1[2],avgp2[2]))
stdp[3] = np.nanstd((avgp1[3],avgp2[3]))
stdp[4] = np.nanstd((avgp1[4]))
stdp[5] = np.nanstd((avgp1[5]))

avganth = np.nanmean((avganth1,avganth2))
stdanth = np.nanstd((avganth1,avganth2))
...
ax.bar(range(len(title_exp)),avgp,yerr=stdp,color='white',edgecolor='k',capsize=4)

if region_name == 'coast':
    ax.set_ylim(bottom=-0.5,top=14)
if region_name == 'grid':
    ax.set_ylim(bottom=-1.5,top=4)
if region_name == 'offshore':
    ax.set_ylim(bottom=-1.5,top=4)

ax.plot(range(len(title_exp)),np.ones((len(title_exp)))*avganth)
ax.fill_between(range(len(title_exp)),avganth+stdanth,avganth-stdanth,alpha=0.3)
...
savename = var_name+'_avg_4years_diff'+region_name+'.png'
